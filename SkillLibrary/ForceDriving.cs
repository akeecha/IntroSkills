...
namespace SkillLibrary
{
	...
	public class ForceDriving : IMistySkill
	{
		...
		{
			_misty.SkillLogger.Log($"ForceDriving : KeyPhraseRecognizedCallback: => {voiceEvent.EventId}");
			_misty.DisplayImage("e_Disoriented.jpg", 1, null);
			_misty.PlayAudio("s_DisorientedConfused.wav", 100, null);
		...
		private void FaceRecCallback(IFaceRecognitionEvent faceRecEvent)
		{
			...
			{
				_misty.SkillLogger.Log($"ForceDriving : FaceRecCallback: => {faceRecEvent.Distance}");
				...
				else 
				{
					...
					{
						_misty.DisplayImage("e_Love.jpg", 1, null);
						_misty.PlayAudio("s_Love.wav", 80, null);
					}
				}
			}
